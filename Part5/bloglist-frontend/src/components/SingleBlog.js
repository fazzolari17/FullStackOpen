  ...
  return (
    <div className='sb_main_container'>
      ...
      <div className='sb_likes_container'>
        <p>{blog.likes} Likes</p><button className='like_btn' onClick={e => handleLike(blog.id)}>Like</button>
      </div>
      ...
      {user.id === blog.user.id && <button className='remove_btn' onClick={e => handleRemove(blog.id)}>Remove</button>}
